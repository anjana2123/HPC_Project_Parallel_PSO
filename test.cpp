...
int main()
{
  ...
  int minIndex = 0, gbestCount = 0;
	double minValue = 0.0, oldValue = 0.0, newValue = 0.0;
  ...
  // Multiplicity "MaxIteration".
	...
	for (int iter = 0; iter < MaxIteration && gbestCount < MaxRepeat; iter++)
	{
		...
		for (int i = 1; i < ParticleCount; i++)
		{
			...
			oldValue = ApplyFunction(functionType, PBest[i]);
			newValue = ApplyFunction(functionType, Xji[i]);
	...
	cout << "Iteration Count: "<< lastIter << " Global Min: "<<FunctionResult[lastIter - 1]<<endl;
